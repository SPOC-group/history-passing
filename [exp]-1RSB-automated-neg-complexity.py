...
tag = "testing20251103-experiments-pop-dyn-neg-complexity"
...
if __name__ == '__main__':
    ...
    df = pd.read_csv("parameters/temperature_search_d1RSB_neg.csv").set_index(["d","p"])
    row = df.loc[(d,T)]
    ...
    steps = int(row.steps)
    for _ in range(reps):
        for rho_temp in np.linspace(to_temp, from_temp, steps):
            ...
            #wandb.init(project="bp-pop-dyn", entity="feeds", config=write_config)
