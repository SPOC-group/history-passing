...
def bribe_rule(args, x_i, x_j, s_i_t, s_i_tp1, s_j_t):
    ...
    if s_j_t is not None:
        ...
        if x_i == BRIBED:
            if s_i_tp1 == ONE:
                return slice(None)  # anything goes... not true
            elif s_i_tp1 == ZERO:
                return slice(0, 0)  # nothing goes

        elif x_i == HONEST:
            if s_i_tp1 == ONE:
                # everything that is larger than half is fine, less if s_j is already one, less if s_i_t is already one (always stay)
                if s_i_t == ONE:
                    return slice(math.ceil(d / 2) - int(x_j or s_j_t) , None)
                elif s_i_t == ZERO:
                    return slice(math.ceil(d / 2 + 0.5) - int(x_j or s_j_t), None)
            elif s_i_tp1 == ZERO:
                if s_i_t == ONE:
                    return slice(None, math.floor(d / 2  + 0.5) - int(s_j_t or x_j) + 1)
                elif s_i_t == ZERO:
                    return slice(
                        None, math.floor(d / 2) - int(s_j_t or x_j) + 1
...
def closing_condition(x_i, x_j, s_i_t, s_i_tp1, s_j_t, args):
    if args.loops is None:
        return slice(None)
    if args.loops == "homogenous":
        return args.rule(args,x_i, x_j, s_i_t, ONE, s_j_t)
    if args.loops == "c-1":
        return args.rule(args,x_i, x_j, s_i_t, s_i_tp1, s_j_t)


def compute_phi(phi, phis_local, args, mask):
    ...
    prob_alife = np.zeros([phi.shape[0]] + [2] + [2] * T + [d] * T)

    for x_k in [BRIBED, HONEST]:
        for x_i in [BRIBED, HONEST]:
            for s_k in product([ZERO, ONE], repeat=T):
                for s_i in product([ZERO, ONE], repeat=T):
                    idx_k_to_i = tuple(
                        [slice(None), 0, x_k, x_i] + list(s_k) + list(s_i)
                    )
                    idx_prob = tuple(
                        [slice(None)] + [x_i] + list(s_i) + [s for s in s_k]
        ...
        for x_k in [BRIBED, HONEST]:
            for x_i in [BRIBED, HONEST]:
                for s_k in product([ZERO, ONE], repeat=T):
                    for s_i in product([ZERO, ONE], repeat=T):
                        idx_k_to_i = tuple(
                            [slice(None), k, x_k, x_i] + list(s_k) + list(s_i)
                        )
    ...
    phi_new = np.zeros_like(phi)
    for x_i in [BRIBED, HONEST]:
        for x_j in [BRIBED, HONEST]:
            for s_j in product([ZERO, ONE], repeat=T):
                for s_i in product([ZERO, ONE], repeat=T):
                    idx_i_to_j = tuple([slice(None), x_i, x_j] + list(s_i) + list(s_j))
                    idx_k = tuple(
                        [slice(None), x_i]
                        + list(s_i)
                        + [
                            rule(args,x_i, x_j, s_i_t, s_i_tp1, s_j_t)
                            for s_i_t, s_i_tp1, s_j_t in zip(
                                list(s_i)[:-1], list(s_i)[1:], list(s_j[:-1])
                            )
                        ]
                        + [closing_condition(x_i, x_j, s_i[-1], s_i[-1], s_j[-1], args)]
                    )
    ...
    for x_i in [BRIBED, HONEST]:
        for x_j in [BRIBED, HONEST]:
            for s_j in product([ZERO, ONE], repeat=T):
                for s_i in product([ZERO, ONE], repeat=T):
                    idx_i_to_j = tuple(
                        [slice(None), 0, x_i, x_j] + list(s_i) + list(s_j)
                    )
                    idx_j_to_i = tuple(
                        [slice(None), 1, x_j, x_i] + list(s_j) + list(s_i)  # order ... ??
                    )
                    Z_ijs[:,s_i[0]] += (
                        phi_ijs[idx_i_to_j]
                        * phi_ijs[idx_j_to_i]
...
def Z_i_(phis_local, mask, args):

    T = args.T
    d = args.d
    rule = args.rule

    prob_alife = np.zeros([phis_local.shape[0], 2] + [2] * T + [d + 1] * (T))
        ...
        for x_i in [BRIBED, HONEST]:
            for s_k in product([ZERO, ONE], repeat=T):
                ...
                for s_i in product([ZERO, ONE], repeat=T):
                    ...
                    idx_prob = tuple(
                        [slice(None), x_i] + list(s_i) + [s for s in s_k]
                    )  # int(s or x_j) the or is the bribe!
                    prob_alife[idx_prob] += phis_local[idx_k_to_i]
    ...
    for x_i in [BRIBED, HONEST]:
        for s_i in product([ZERO, ONE], repeat=T):
            idx_k = tuple(
                ...
                + list(s_i)
                + [
                    rule(args,x_i, None, s_i_t, s_i_tp1, None)
                    for s_i_tp1, s_i_t in zip(list(s_i)[1:], list(s_i)[:-1])
                ]
                + [closing_condition(x_i, None, s_i[-1], s_i[-1], None, args)]
            )
        ...
        for x_j in [BRIBED, HONEST]:
            for s_j in product([ZERO, ONE], repeat=T):
                for s_i in product([ZERO, ONE], repeat=T):
                    ...
                    idx_j_to_i = tuple(
                        [slice(None), 1, x_j, x_i] + list(s_j) + list(s_i)
                    )
            ...
            for s_j in product([ZERO, ONE], repeat=T):
                for s_i in product([ZERO, ONE], repeat=T):
                    ...
                    Z_ijs += (
                        ...
                        * phi_locals[idx_j_to_i]
                        * 1/ args.observables.measure(x_i, x_j, s_i, s_j).prod()
                    )[:, np.newaxis] * args.observables.values(x_i, x_j, s_i, s_j)[
                        np.newaxis, :
                    ]
    return Z_ijs
...
class Observables:
    ...
    def values(self, x_i, x_j, s_i, s_j):
        return np.array([f(x_i, x_j, s_i, s_j) for f in self.funcs])

    def measure(self, x_i, x_j, s_i, s_j):
        values = self.values(x_i, x_j, s_i, s_j)
...
def reaches_consensus_in_T_steps(G,s,x,T):
    result = evolve_dynamics(G=G,s0=s.astype(bool),x=x.astype(bool),T=T)
...
def history_passing(args,phi=None):
    ...
    phi = rs.uniform(size=tuple([d * n] + [2, 2] + [2] * 2 * args.T))
    ...
    # set all messages with bribed state to zero
    indices = np.indices(phi.shape)
    mask = np.logical_and(indices[1] == 0,indices[2] == 0)
    phi[~mask] = 0.0
    phi = normalize(phi)
    x = np.zeros(n)
    ...
    while not reaches_consensus_in_T_steps(G,s_estimate,x,T) or t < 5000:
        for _ in range(args.bp_its):
            new_shape = tuple([slice(None), None, None, slice(None), None] + [None] * (2* (T-1)))
            phi_bias = phi * bias[edges_array[:,0]][new_shape]
            
            phis_local = phi_bias[neighbours_dm1]
            phi_new = compute_phi(phi_bias, phis_local, args, mask)
            phi = (1 - args.damp) * phi + args.damp * phi_new 
        ...
        result = evolve_dynamics(G=G,s0=s_estimate.astype(bool),x=x.astype(bool),T=T)
        result_random = evolve_dynamics(G=G,s0=rs.permutation(s_estimate).astype(bool),x=x.astype(bool),T=T)
